...
import { uploadPageImage } from '../../../../actions/page';

const PrivacyType = [
    { type: 0, title: "Public" },
    { type: 1, title: "Private" }
]
const SocialCreatePage = (props) => {
    ...
    const handleCreateGroup = async () => {
        if (state.groupName.length < 1) {
            setState({
                ...state,
                groupNameErr: true,
                groupNameErrMsg: "Please enter page name."
            })
            return 1;
        }
        ...
        if (state.image.length < 1) {
            Toast.show({
                ...
                text2: "Please select a page image.",
                type: "error",
                position: "top"
            });
            return 1;
        ...
        if (state.coverImage.length < 1) {
            Toast.show({
                text1: constants.AppConstant.Hypr,
                text2: "Please select a page cover image.",
                type: "error",
                position: "top"
            });
            return 1;
        ...
        await props.dispatch(uploadPageImage(state.imageFull, state.coverImageFull, payload))
    }
    return (
        <>
...
export default connect(mapStateToProps, mapDispatchToProps)(SocialCreatePage);